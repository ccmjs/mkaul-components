          ...
          inner: [
                ...
                { tag: 'h3', inner: [
                  ...
                  { tag: 'span', class: 'counter', inner: '0' },
                    ', &nbsp; Max: &nbsp; ',  // html.main.inner.1.inner.3.min
                  { tag: 'input', class: 'max', type: 'number', min: 5, value: "%max%", oninput: '%oninput%' },
                    ' &nbsp; Frei: &nbsp; ',
            ...
            { class: 'button_container', inner: [ { tag: 'button', class: 'emergency', inner: 'Emergency', onclick: '%emergency%' } ] },
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        class Garage {  // single source of truth for max and all counters
          ...
          async addCar( car ){
            if ( garage.countCars() >= garage.max ){
              ...
              if ( ! self.json_format || self.json_format === "false" ){
                ...
                if ( reply === null ) return; // break; do not enter
                if ( reply ){
          ...
          async removeAll(){
            for ( const car of Object.values( this._space_index )){
              this.removeCar( car );
            }
          }
          async removeCar( car ){
        ...
        class Car {
          constructor( spec ){
            if ( spec ){
              if ( typeof spec === 'string' ){
                [ this.nr, this.timer, this._duration, this._price, this._hash, this._color, this.space,
                  this.client_category, this.vehicle_type, this._license, this._beginTime ] = spec.split("/");
                [ 'nr', 'timer', '_duration', '_price', 'space', '_beginTime' ].forEach( prop => this[prop] === '_' ? '_' : parseInt(this[prop]));
          ...
          enter(){
            this._inGarage = true;
            this._duration = 0;
            this.timer = (new Date()).getTime(); // invariant: this.timer >= begin time of parking
          }
          leave() {
            this._inGarage = false;
          ...
          beginTime(){
            if ( this._beginTime ) return new Date(parseInt(this._beginTime));
            return simulationTime( this.timer );
          }
          end() {  // invariant: this.timer is always the begin time of parking
            const endTime = parseInt(( this._beginTime || this.beginTime() )) + parseInt( this._duration );
          ...
          toObject(){
            return {
              ...
              begin: this._beginTime || this.beginTime(),
          ...
          toArray(){
            const list = [ this.nr, this.timer, this._duration, this.price(), this.hash(), this.color(), this.space, this.client_category, this.vehicle_type, this.license, this._beginTime || this.beginTime(), self.name ];
        ...
        function emergency( event ){
          csv_post_event( 'emergency', new Date( simulationNow() ).toLocaleString() );
          garage.removeAll();
        }
