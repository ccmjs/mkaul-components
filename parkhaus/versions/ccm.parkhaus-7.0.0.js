    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        class Car {
          constructor( spec ){
            if ( spec ){
              [ this.nr, this.timer, this._duration, this._price, this._hash, this._color, this.space ] = spec.split("/");
          ...
          toArray(){
            return [ this.nr, this.timer, this.duration(), this.price(), this.hash(), this.color(), this.space ].map(x => (x || '_').toString() );
          }
          tooltip(){
