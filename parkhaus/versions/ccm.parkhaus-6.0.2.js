  ...
  var component  = {
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        const header = {};
        ['counter','Max','open_from','open_to' ].forEach( className => {
        ...
        const counter_span = main_elem.querySelector("span.counter");

        class Counter {
          constructor( init ){
            this._value = init;
          }
          increment() { this._value += 1; counter_span.innerText = counter.toString(); };
          decrement() { this._value -= 1; counter_span.innerText = counter.toString(); };
        ...
        async function enter( e ) {
          e.preventDefault();
          if ( counter.value() >= parseInt( self.Max ) ){
