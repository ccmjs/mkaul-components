    ...
    Instance: function () {
      ...
      this.start = async () => {
          ...
          add_image( more ){
            ...
            if ( more > 0 ){
              ...
              for ( let i = this._max; i > this._max + more; i-- ){
                garage_div.removeChild( garage_div.lastElementChild );
              }
            }
            this.rerender();
            // garage_div.innerHTML += '';
        ...
        class Car {
          ...
          toArray(){
            return [ this.nr, this.timer, this.duration(), this.price(), this.hash(), this.color(), this.space, this.client_category, this.license ].map(x => (x || '_').toString() );
          }
          tooltip(){
