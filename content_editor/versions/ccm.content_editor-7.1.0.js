 ...
 * @changes
 * version 7.1.0  base64 encoding of pasted images, add embedded tables
 * version 7.0.0  migrate to ccm v25
...
( function () {
  ...
  var component = {
    ...
    config: {
      ...
      data: {
        "store": [ "ccm.store", { "name": "content_editor", "url": "http://ccm2.inf.h-brs.de" } ],
        "key": "test"
      },
      ...
      ccm_save: 'https://ccmjs.github.io/ccm/versions/ccm-25.1.0.min.js',  // for saving content
      content_url: 'https://ccmjs.github.io/akless-components/content/versions/ccm.content-5.4.0.js',
      ...
      json_builder: [ "ccm.component", "https://ccmjs.github.io/akless-components/json_builder/versions/ccm.json_builder-2.0.0.js", {
        ccm: "https://ccmjs.github.io/ccm/versions/ccm-25.1.0.min.js",
        "html.inner.1": "",
        "directly": true
      } ],
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        Object.keys( self ).forEach( key => {
          const match = key.match(/^default-?(\w+)/);
          if ( match ){
            const group = match[ 1 ];
            if ( ! dataset.inner ) dataset.inner = `<${group}>${ self[ key ] }</${group}>`;
          }
        });
        if ( ! dataset.inner ) dataset.inner = self.default ? self.default: '<p>Edit here</p>';
        ...
        editor_div.onpaste = function (e) {
          ...
          const insertAtSelection = newElement => {
            let range = getSelectionRange();
            if ( range ) {
              range.insertNode( newElement );
              // Array.from( newElement.childNodes ).forEach( child => range.insertNode( child ) );
              // range.setEndAfter( newElement );
              // getSelection().collapse( newElement,0 );
            } else {
            ...
            // newElement.focus && newElement.focus();
          };
          ...
          [...e.clipboardData.items].forEach(async item => {
            switch (item.type) {
              case 'text/plain':
                ...
                // process pastedText here only if there is no HTML companion
                if ( e.clipboardData.items.length === 1 ) insertAtSelection( document.createTextNode( pastedText ) );
                break;
              case 'text/html':
                updateInPageAnchors();
                const pastedHTML = e.clipboardData.getData('text/html');
                // process pastedHTML here
                insertAtSelection( $.html( pastedHTML ) );
                // editor_div.appendChild( $.html( pastedHTML ) );
                updateData();
                e.preventDefault();
                break;
              case 'text/rtf':
              ...
              case 'image/png':
                ...
                pastedImage.src = await toBase64( file );
                if ( pastedImage.height ){
                  pastedImage.height /= 2;
                  pastedImage.style.height = pastedImage.height;
                }
                if ( pastedImage.width ){
                  pastedImage.width /= 2;
                  pastedImage.style.width = pastedImage.width;
                }

                insertAtSelection( pastedImage );
        ...
        async function toolbarClickListener(e) {
          ...
          switch (command) {
            ...
            case 'table':
              const [ row, col ] = prompt('Number of rows? columns?','3,2').split(',');
              const head = { tag: 'tr', style: 'border:1px solid;text-align: center;padding: 0.4rem auto;color:white;background-color:black;', inner: numbersTo(col).reduce(a=>{a.push({ tag: 'th' }); return a}, []) };
              const rows = { tag: 'tr', style: 'border:1px solid;text-align: center;padding: 0.3rem auto;', inner: numbersTo(col).reduce(a=>{a.push({ tag: 'td', style: 'border:1px solid;text-align: center;padding: 0.2rem auto;' }); return a}, []) };
              const body = numbersTo(row).reduce(a=>{a.push($.clone(rows));return a},[]);
              const table_structure = { inner: { tag: 'table', style: 'width:100%;border-collapse:collapse;', inner: [ head, ...body ] } };
              execCommand('insertHTML', false, ($.html( table_structure )).innerHTML );
              break;
        ...
        function getSelectionRange() {
          ...
          if (window.getSelection) {
            ...
            if ( $.isSafari() ){
              ...
              try {
                if ( range && ! range.intersectsNode( editor_div ) ) return null;
              } catch (e) {
                return null;
              }
            }
        ...
        async function save() {
          ...
          if ( $.isDatastore( self.data.store ) ) dataset.user = user; // protocol who has written finally to store
          if ( ! dataset.key ) dataset.key = self.data.key;   // same store
      ...
      function getIndex( url ){
        const match = url.match(/ccm\.([.\w\d-]+)\.js/i);
      ...
      this.getValue = () => {
        ...
        // const blobs = Array.from( result.inner.matchAll(/src="(blob[^"]+)/ig) );
        //
        // await Promise.all( blobs.map( ( match, i ) => new Promise( ( resolve ) => {
        //
        //   const img = document.createElement('img');
        //   img.src = match[1];
        //   self.root.parentElement.appendChild( img );
        //   const imgsrc = new Blob( [ match[1] ], { type : "image/bmp" } );
        //
        //   fetch( match[1] ).then(function(response) {
        //     if (!response.ok) {
        ...
        //     self.root.parentElement.appendChild( img2 );
        //
        //     result.inner = result.inner.slice(0,match.index) + ' src="' + reader.result + result.inner.slice(match.index+match[1].length, result.inner.length);
        //     resolve( result );
        //   });
