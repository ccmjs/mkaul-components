 ...
 * @changes
 * version 4.8.0 Three more buttons: include HTML pages, remove toolbar and editor itself
...
( function () {
  ...
  var component = {
    ...
    config: {
      ...
      draw_svg: [ "ccm.component", "https://ccmjs.github.io/mkaul-components/draw_svg/versions/ccm.draw_svg-2.0.0.js" ],
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        async function toolbarClickListener(e){
          ...
          switch (command){
            ...
            case 'makeexternallink':
              ...
              const range = getSelectionRange();
              if ( range ){
                document.execCommand('insertHTML', false, `<a href="${uri}" target="_blank">${range.toString()}</a>`);
              } else {
        ...
        async function insertComponent({ component, config }){
          ...
          //   || editor_div.parentNode.getSelection && editor_div.parentNode.getSelection()
