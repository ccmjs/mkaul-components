 ...
 * @changes
 * version 1.1.0 23.07.2019 improve default values in template
 *    add task config to overwrite DMS config
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        [...this.element.querySelectorAll('.task')].forEach( async ( taskDiv, i ) => {
          ...
          const config1 = await self.ccm.get( { name: store, url }, task.appid );
          const config2 = $.format( self.templates.ignore, { name: self.name, nr: i+1 });
          const config3 = { root: taskDiv, user: self.user };

          // merge all config data into a single object
          const config4 = Object.assign( config1, config2, config3, task_config );
