...
( () => {
  ...
  const component = {
    ...
    Instance: function () {
      ...
      /**
       * store opened at runtime, not beforehand
       * @type {ccm.store}
       */
      let store;
      ...
      this.start = async () => {
        ...
        async function render(){
          ...
          if ( stateOpen ){
            ...
            // fetch dataset
            store = await self.ccm.store( self.store );
            if ( $.isDatastore( store ) ) store.onchange = self.start;
            dataset = await store.get( self.name );  // await $.dataset( store );
