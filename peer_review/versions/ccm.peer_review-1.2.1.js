  ...
  const component = {
    ...
    config: {
      ...
      html: {
        ...
        grade: { tag: 'fieldset', inner: [
            ...
            { tag: 'input', type: 'radio', value: "2" },
            { tag: 'input', type: 'radio', value: "3" },
            { tag: 'input', type: 'radio', value: "4" },
            { tag: 'input', type: 'radio', value: "5" }
        ]},
        no_review: {
          class: 'nothing',
        ...
        warning: "New update %peer% of peer solution available. Review of %review% is outdated."
      },
    ...
    Instance: function () {
      ...
      this.ready = async () => {
        ...
        // add repetitive attributes and labels to radio buttons
        if ( self.html.grade ){
          grade = $.clone( self.html.grade );
          const radiosWithLabels = [];
          grade.inner.forEach( elem => {
            radiosWithLabels.push( elem );
            if ( elem.tag === 'input' && elem.type === 'radio' ){
              // add attributes
              elem.name = '%task%_%category%_%other%';
              elem.onclick = '%onclick%';
              elem.id = '%category%_' + elem.value;
              // add labels
      ...
      this.start = async () => {
        ...
        if ( this.user ){
          ...
          if ( dataset && dataset.peers ){
            ...
            const peer_Objects = await this.peer_texts.store.get({ _id: { $regex: regex }});
            // const peer_Objects = await $.dataset( this.peer_texts.store, { _id: { $regex: regex } } );
            ...
            if ( peer_Objects ){
              peer_Objects.forEach( peerObject => {
                ...
                Object.entries( this.task_formats ).forEach( ( [ task, format ] ) => {
                  ...
                  if ( peerObject.updated_at > dataset.updated_at ){
                    $.append( single, $.html( self.html.warning, { peer: peerObject.updated_at, review: dataset.updated_at } ) );
                  }

                  if ( peerText ){
                    if ( ! dataset.reviews[ other ] ) dataset.reviews[ other ] = {};
                    if ( ! dataset.reviews[ other ][ task ] ) dataset.reviews[ other ][ task ] = {};
                    if ( ! dataset.reviews[ other ][ task ].source ) dataset.reviews[ other ][ task ].source = peerText;
                    ...
                    switch ( format ) {
                      ...
                      case 'quill':
                        this.quill.start({
                          ...
                          data: dataset.reviews[ other ][ task ].source,
                          onchange: function(){
                            dataset.reviews[ other ][ task ].source = this.getValue();
                          }
                        });
                    ...
                    if ( dataset.reviews[ other ][ task ].review ) review.value = dataset.reviews[ other ][ task ].review;

                    // update: copy new version of review into dataset
                    review.addEventListener('input', e => {
                      dataset.reviews[ other ][ task ].review = review.value;
                    });
                    ...
                    if ( self.grading_categories ){
                      self.grading_categories.forEach( category => {
                        ...
                        if (dataset.reviews[ other ][ task ][ category ] ){
                            const radio = myGrade.inner[ 2 * parseInt( dataset.reviews[ other ][ task ][ category ] ) - 1 ];
                        ...
                        const onclick = function( event ){
                          ...
                          dataset.reviews[ other ][ task ][ category ] = this.value;
                        };

                        const radio_buttons = $.html( myGrade, { task, category, other, onclick } );
