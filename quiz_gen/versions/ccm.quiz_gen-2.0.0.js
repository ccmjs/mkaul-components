 ...
 * @changes
 * version 2.0.0 19.01.2020 initial build
 * TODO: unit tests
  ...
  const component = {
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        const store = await this.ccm.store( { url: this.store.url, name: this.store.name, login: true, user: user } );

        dataset = await $.dataset( { store: store, key: { _id: { $regex: this.store.query } } } );
