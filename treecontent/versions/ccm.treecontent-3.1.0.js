 ...
 * @changes
 * version 3.1.0 06.02.2019 position cursor at the end of input
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        sort_button.addEventListener('click', e => {
          sort( this.element.querySelector('.tree li') );
        });

        if ( self.sorted ) sort( this.element.querySelector('.tree li') );

        let count = 1;
        const leaveListener = async event => {
          console.log( `start(${count++})` );
          $.wait(400, await self.start());
        };
        this.root.removeEventListener('mouseleave', leaveListener);
        this.root.addEventListener('mouseleave', leaveListener);
        ...
        function sort( clickedNode ){
          ...
          const sortedIds = Object.keys( itemValue ).sort(
            (a,b)=>isEmpty(rowIndex[parent.id],parent)?-9999:itemValue[a]-itemValue[b]);
